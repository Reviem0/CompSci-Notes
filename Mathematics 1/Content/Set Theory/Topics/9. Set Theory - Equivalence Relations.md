...
## Equivalence Classes
An **equivalence class** is a subset of elements that are all related to each other under a given equivalence relation. If we have an equivalence $R$ on a set $S$, then the equivalence class of an element $a \in S$, denoted $[a]$, is defined as:
